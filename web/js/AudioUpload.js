...
/* =====================================================================
H3AudioUpload - compact refined edition (v5)
...
const NODE_CLASS = "H3AudioUpload";
            ...
            if (this.pau_built) {
                ...
                if (fn) {
                    loadAndRender(this, fn).catch((err) =>
                        console.error("[H3AudioUpload] restore failed:", err)
                    );
                } else {
                    renderEmpty(this);
                }
...
function buildUI(node) {
    ...
    if (fn) {
        loadAndRender(node, fn).catch((err) =>
            console.error("[H3AudioUpload] initial load failed:", err)
        );
    } else {
        renderEmpty(node);
    }
...
function restoreOriginal(node) {
    ...
    console.log("[H3AudioUpload] restored original: trim cleared (0 / -1)");
}
