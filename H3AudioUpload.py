...
class H3AudioUpload:
    ...
    def load_audio(self, audio_filename="", trim_start=0.0, trim_end=-1.0):
        if not audio_filename:
            ...
            if had_trim:
                raise ValueError(
                    "H3AudioUpload: trim range is set but no audio file "
            ...
            raise ValueError(
                "H3AudioUpload: no audio has been uploaded or recorded. "
        ...
        if file_path is None:
            raise FileNotFoundError(
                f"H3AudioUpload: audio file not found in input dir: {audio_filename}"
            )
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    f"H3AudioUpload: failed to decode audio file {file_path}: {exc}"
                ) from inner
        ...
        if total_samples <= 0:
            raise RuntimeError("H3AudioUpload: audio file is empty.")
...
NODE_CLASS_MAPPINGS = {
    "H3AudioUpload": H3AudioUpload,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "H3AudioUpload": "H3 Audio Upload",
}
